...
import { ConfidenceLevel, ResultsProps } from "../types";
import { verdictConfig } from "@/utils/verdictConfig";
...
const Results: React.FC<ResultsProps> = ({ results }) => {
  ...
  const config = verdictConfig[confidence as ConfidenceLevel] || {
                ...
                {sortedSources.map((source, index) => (
                  ...
                  >
                    ...
                    <p className="text-[15px]">
                      Domain Type:{" "}
                      <span
                        className={`p-0 rounded-full text-sm font-semibold ${source.domain_type === "trusted"
                          ? " text-green-500"
                          : " text-yellow-300"
                          }`}
                      >
